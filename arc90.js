...
/**
 * Public constructor.
 * @param {document} doc     The document to parse (must be a clone to avoid modifying the original).
 * @param {Object}   options The options object.
 */
function Readability(doc, options) {
  // Ensure the input document is a clone to prevent any modification to the original DOM
  if (!doc || !doc.documentElement) {
    throw new Error("First argument to Readability constructor should be a document object.");
  }
  options = options || {};

  this._doc = doc; // Use the cloned document for all operations
...
Readability.prototype = {
   ...
   * Operates only on the cloned document.
   * @param Element
   * @return void
   */
...
var clonedDocument = window.document.cloneNode(true);
var options = {
  debug: false, 
  maxElemsToParse: 0, 
  nbTopCandidates: 10, 
};
var reader = new Readability(clonedDocument, options);
