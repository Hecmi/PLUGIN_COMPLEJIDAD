...
!function(A, e) {
    ...
    "use strict";
    console.log("html2canvas adaptado listo!");
    /*! *****************************************************************************
      ...
      , ue = function(A, e) {
        if (18 === e.type) {
            ...
            if (typeof t !== 'function') {
                console.log("Not valid function")
                return;
            }
...
function calculate_colorfulness(canvas) {
    const img = canvas.getContext("2d").getImageData(0, 0, canvas.width, canvas.height);
    const pixels = img.data;

    // Cada pixel tiene 4 valores (r, g, b, a)
    const p = pixels.length / 4;
    ...
    // Calcular las medias (mu) para  r_g and y_b
    let mean_rg = rg.reduce((sum, value) => sum + value, 0) / p;
    let mean_yb = yb.reduce((sum, value) => sum + value, 0) / p;

    // Calcualar las varianzas
    let variance_rg = rg.reduce((sum, value) => sum + (value - mean_rg) * (value - mean_rg), 0) / (p - 1);
    let variance_yb = yb.reduce((sum, value) => sum + (value - mean_yb) * (value - mean_yb), 0) / (p - 1);

    // Calcular la desviación estándar (sigma)
    let sigma_rg = Math.sqrt(variance_rg);
    let sigma_yb = Math.sqrt(variance_yb);

    // Calcular la desviación estándar combinada  (sigma_r_g_y_b)
    let sigma_rgyb = Math.sqrt(sigma_rg * sigma_rg + sigma_yb * sigma_yb);

    // Calcular la media combinada (mu_r_g_y_b)
    let mu_rgyb = Math.sqrt(mean_rg * mean_rg + mean_yb * mean_yb);

    // Calcular la coloridad, normalizando con la división entre 255
...
html2canvas(document.body, {
    ...
    colorfulness = calculate_colorfulness(captured_canvas);

    if (colorfulness < 0.3) {
        console.log("La página tiene pocos colores");
    } else if (colorfulness < 0.6) {
        console.log("Buena variabilidad de colores");
    } else {
        console.log("Demasiados colores");
    }
