// Página de prueba: https://arngren.net/
...
function calculate_entropy(canvas) {
    ...
    const total_pixels = pixels.length / 4

    // Recorrer cada uno de los píxeles y modificar su frecuencia
    for (let i = 0; i < pixels.length; i += 4) {
        ...
        // Crear un color, centrándose únicamente en los valores RGB
        const colorKey = `${r},${g},${b}`;
        
        // Incrementar la frecuencia del color
        if (color_frequency[colorKey]) {
            color_frequency[colorKey]++;
        } else {
            color_frequency[colorKey] = 1;
        }
    }

    let entropy = 0;

    // La cantidad de colores es posibles es equivalante a 256^3
    // Pero para trabajarlo en bits, se aplica el logaritmo en base 2, resultando 
    // en 24 bits activos
    let max_value = 24; //Math.log2(Math.pow(256, 3));
    for (let color in color_frequency) {
        // Calcular la probabilidad del color
        const probability = color_frequency[color] / total_pixels;

        // Aplicación de la fórmula de Shannon para determinar la entropía 
        // según la probabilidad de los colores. 
        // Se utiliza logarítmo en base dos para medir la probabildiad en bits
        entropy -= ((probability * Math.log2(probability)) / max_value);
    }

    // Ordenar los colores según su frecuencia
    const sorted_colors = Object.entries(color_frequency)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5);
...
html2canvas(document.body, {
    ...
    scale: 1,
    onerror: function(e) {
        console.warn("Se ha producido un error de CORS. Se ignorará la imagen:", e);
        return true;
    }
    ...
    // Draw the image (all the DOM) in the canvas
    ctx.drawImage(captured_canvas, 0, 0);
    entropy = calculate_entropy(captured_canvas);

    // Evaluar la entropía y mostrar un mensaje en función del valor
    if (entropy >= 0.3 && entropy <= 0.7) {
        console.log("Buena entropía de color. Balanceado visualmente");
    } else if (entropy < 0.3) {
        console.log("Baja entropía de color. El diseño puede ser muy simple o monótono");
    } else {
        console.log("Alta entropía de color. El diseño resulta demasiado complejo");
    }
